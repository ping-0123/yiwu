alter TABLE chenkuserdb1.skt23 add COLUMN settleStatus varchar(32);
CREATE 
	or REPLACE
VIEW `vcheck_ins` AS
    SELECT 
        ...
        `chenkuserdb1`.`skt23`.`appointed` AS `appointed`,
    ...
    FROM
        `chenkuserdb1`.`skt23`;
UPDATE vcheck_ins set settleStatus = if(flag, 'SETTLED','UN_SETTLED');
SELECT * from vcheck_ins ORDER BY sf_create_time desc;