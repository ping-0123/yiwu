...
public class OrderPayedMethodYzw  extends BaseYzwEntity{
	...
	@JoinColumn(name="orderId", foreignKey=
			@ForeignKey(value=ConstraintMode.NO_CONSTRAINT, name="fk_orderPayedMethod_order_id"))
	private OrderYzw order;
	
	@ManyToOne(fetch=FetchType.LAZY)
	@JoinColumn(name="payedMethodId", foreignKey=
			@ForeignKey(value=ConstraintMode.NO_CONSTRAINT, name="fk_orderPayedMethod_payedMethod_id"))
