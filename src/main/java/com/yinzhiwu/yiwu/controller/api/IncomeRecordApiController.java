...
public class IncomeRecordApiController extends BaseController {
	...
	public YiwuJson<List<ShareTweetIncomeRecordApiView>> getShareTweetListFaster(
			...
			@ApiParam(value = "事件类型Id数组;"
					...
					+ "10008:用基金支付定金,10009:用佣金支付定金, 10010:产生利息, 10011:提现,"
					+ "10027:预约, 10030:取消预约, 10028:签到（预约后), 10029:签到（未预约）,  不输入:表示全部]", required = true) int[] eventTypeIds,
			@ApiParam(value = "关系类型Id数组: Id表示事件当事人和观察者(本接口环境中同收益人)的关系;"
					+ "10015：当事人和当事人,10016：当事人和他上一级,10017：当事人和上两级, 不输入:表示全部", required = true) int[] relationTypeIds,
			@ApiParam(value = "收益类型Id数组； 10012:经验收益类型, 10013:基金收益类型, 10014：佣金收益类型 , 不输入:全部类型", required = true) int[] incomeTypeIds) {
		try {
	...
	public YiwuJson<Long> getCount(@ApiParam(value = "id of distributer", required = true) int observerId,
			...
			@ApiParam(value = "id of the relation betweet event subject and observer; "
					+ "10015：本人和本人,10016：本人上一级,10017：本人和上两级, -1 表示全部", required = true) int relationTypeId,
			@ApiParam(value = "收益类型Id； 10012:经验收益类型, 10013:基金收益类型, 10014：佣金收益类型 , -1:全部类型", required = true) int incomeTypeId) {
		List<String> properties = new ArrayList<>();
		List<Object> values = new ArrayList<>();
		properties.add("benificiary.id");
	...
	public YiwuJson<Integer> getCountByIncomeTypes(@ApiParam(value = "分享者Id", required = true) int distributerId,
			@ApiParam(value = "收益类型Id数组: 10012:经验收益类型， 10013：基金收益类型, 10014:佣金收益类型", allowableValues = "10012,10013,10014", required = true) int[] incomeTypeIds) {
		try {
			return new YiwuJson<>(
