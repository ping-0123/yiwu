...
//		
		...
		if(fundsPayed> 0f){
			fundsDepoist = new DepositEvent(distributer, EventType.PAY_DEPOSIT_BY_FUNDS, fundsPayed, null);
			incomeEventService.save(fundsDepoist);
		}
		if(brokeragePayed>0f){
			brokerageDeposit = new DepositEvent(distributer, EventType.PAY_DEPOSIT_BY_BROKERAGE, brokeragePayed, null);
