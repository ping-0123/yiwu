...
public class RevenueServiceImpl implements RevenueService {
	...
	public Object getMonthlyRevenue(int year, int month, int districtId, int productTypeId) {
		...
		for (Object[] objs : revenues) {
			try {
				revenue[getRowIndex((Date) objs[2])][getCloumnIndex((int) objs[0], stores)].setAmount((double) objs[3]);
			} catch (Exception e) {
		...
		for (int j = 0; j < cols; j++) {
			PlanRevenue p = prDao.findStoreMonthlyPlanRevenue(stores.get(j).getId(), productTypeId, year, month);
			plans[j] = p;
